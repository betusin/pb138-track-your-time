...
export const ProjectItem = ({id, name, hourly_rate, isActive, customer}: IProjectType) => {
  ...
  return (
    <div className={`project-item ${!isActive && 'project-item--not-active'} p1 m1`}>
      <Link to={`/project/${id}`} className="project-item__name"><strong>{name}</strong></Link>
      ...
      <div className="project-item__settings">
        ...
        {wantToRemove ? (
          <img className={`icon icon--inverse ${isActive && 'icon--active_inverse'}`} title='Yes, I want to delete the project' onClick={() => deleteProject()} src={`/assets/delete-${isActive ? 'lime' : 'antracit'}.svg`} />
        ) : (
          <img className="icon" title='Delete the project' onClick={() => setWantToRemove(!wantToRemove)} src={`/assets/delete-${isActive ? 'antracit' : 'lime'}.svg`} />
        )}
      </div>
    </div>
  );
